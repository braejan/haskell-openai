...
-- testExtraFields = testCase "Deserialization of JSON with extra fields to Choice should result in an error." $ do
--   let input = "{\"text\":\"hello\",\"index\":0,\"logprobs\":[0.0,0.0],\"finish_reason\":\"length\",\"extra_field\":\"extra_value\"}"
--       choice' = decode (fromString input) :: Maybe Choice
--   assertBool "Test 5: Deserializing JSON with extra fields to Choice should result in an error." (isNothing choice')